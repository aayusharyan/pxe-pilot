...
- **Optional:** `PXE_UBUNTU_ISO_URL` (URL to the Ubuntu live-server ISO; required for Ubuntu 24.04 casper boot), `PXE_UKI_URL` (where iPXE chains the UKI from; default `tftp://${next-server}/uki.efi`, override when DHCP siaddr is not populated - e.g. some Unifi setups), `DATABASE_PATH` (default: pxe.db), `PXE_TFTP_ENABLED` (1/true/yes to run TFTP in container), `PORT`, `ADMIN_API_KEY`
